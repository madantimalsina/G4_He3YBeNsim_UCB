...

{
  ...
  gPad->SetLogy(1);
  gPad->SetGrid();
  TH1D* hist1 = (TH1D*)f.Get("Ek_f_W");
  hist1->SetTitle("Neutron K.E. coming out tungsten pig that hits He3 tube (2 in PE)"); 
  ...
  gPad->SetLogy(1);
  gPad->SetGrid();
  TH1D* hist2 = (TH1D*)f.Get("Ek_f_W_detected");
  hist2->SetTitle("Neutron K.E. coming out tungsten pig that detected (2 in PE)"); 
